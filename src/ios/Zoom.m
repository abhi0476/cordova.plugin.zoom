    ...
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        ...
        if (ms)
        {
            ...
            else
            {
                ...
                if (zoomAccessToken == nil || ![zoomAccessToken isKindOfClass:[NSString class]] || [zoomAccessToken length] == 0 ||
            ...
            // customer_key
            if ([options objectForKey:@"customer_key"] != nil) {
                param.customerKey = options[@"customer_key"];
            }
            // Start meeting.
            MobileRTCMeetError response = [ms startMeetingWithStartParam:param];
            if (DEBUG) {
                NSLog(@"start a meeting res:%lu", response);
            }
        }
    });
}
...
- (void)startInstantMeeting:(CDVInvokedUrlCommand*)command
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^(void) {
        ...
        if (ms)
        {
            ...
            MobileRTCMeetError response = [ms startMeetingWithStartParam:param];
            if (DEBUG) {
                NSLog(@"start an instant meeting res:%lu", response);
            }
        }
    });
}
...
- (void)onMobileRTCLoginResult:(MobileRTCLoginFailReason)returnValue;
{
